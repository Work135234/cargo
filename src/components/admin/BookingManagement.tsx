...
const BookingManagement: React.FC = () => {
  ...
  const handleScheduleBooking = async () => {
    ...
    setError('');
    try {
      // Use admin endpoint if user is admin, else use customer endpoint
      const user = JSON.parse(localStorage.getItem('user') || '{}');
      const isAdmin = user && user.role === 'Admin';
      const url = isAdmin
        ? 'http://localhost:5000/api/admin/schedule-booking'
        : 'http://localhost:5000/api/bookings/schedule';
      const body = isAdmin
        ? {
          bookingId: selectedBooking._id,
          pickupDate: scheduleData.scheduledDate,
          deliveryDate: scheduleData.estimatedDelivery,
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-4 mt-2 text-sm">
                  ...
                  <div>
                    ...
                    <p><strong>Scheduled From:</strong> {formatDate(selectedBooking.scheduledDate || '')}</p>
                    <p><strong>Scheduled To:</strong> {formatDate(selectedBooking.estimatedDelivery || '')}</p>
                  </div>
                </div>
              </div>
