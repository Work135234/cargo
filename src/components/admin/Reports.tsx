...
import { BarChart3, Download, Calendar, Loader2, TrendingUp, Package, DollarSign, FileText } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from '@/components/ui/card';
import { Alert, AlertDescription } from '@/components/ui/alert';
...
const Reports: React.FC = () => {
  ...
  const exportToCSV = () => {
    ...
    if (filters.reportType === 'bookings') {
      csvContent = 'Booking ID,Customer Name,Email,Pickup Address,Delivery Address,Distance (km),Weight (kg),Transport Mode,Fare ($),Status,Created Date\n';
      ...
      if (reportData.revenueByMonth) {
        Object.entries(reportData.revenueByMonth).forEach(([month, revenue]) => {
          const monthBookings = reportData.bookings?.filter((b: any) =>
            new Date(b.createdAt).toLocaleDateString('en-US', { month: 'long', year: 'numeric' }) === month
          ) || [];
          const avgFare = monthBookings.length > 0 ?
            (revenue / monthBookings.length).toFixed(2) : '0.00';

          csvContent += `"${month}","${revenue}","${monthBookings.length}","${avgFare}"\n`;
        });
      }
    }
  ...
  const exportToPDF = () => {
    ...
    const url = URL.createObjectURL(blob);
    link.setAttribute('href', url);
    link.setAttribute('download', `${filters.reportType}_report_${new Date().toISOString().split('T')[0]}.txt`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    ...
    <div className="space-y-6">
      ...
      {reportData.bookings && reportData.bookings.length > 0 && (
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              {reportData.bookings.slice(0, 10).map((booking: any) => (
                <div key={booking._id} className="flex justify-between items-center p-2 border rounded">
                  <div>
                    <p className="font-medium">Booking #{booking._id.slice(-6)}</p>
                    <p className="text-sm text-gray-600">
          ...
          <CardContent>
            <div className="space-y-2">
              {reportData.topCustomers.slice(0, 5).map((customer: any, index: number) => (
                <div key={customer._id} className="flex justify-between items-center p-2 border rounded">
                  <div>
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button onClick={exportToCSV} disabled={!reportData || Object.keys(reportData).length === 0}>
                <Download className="mr-2 h-4 w-4" />
                Export CSV
              </Button>
              <Button onClick={exportToPDF} disabled={!reportData || Object.keys(reportData).length === 0} variant="outline">
        ...
        <CardContent>
          ...
          {loading ? (
            ...
            <div>
              {filters.reportType === 'bookings' ? renderBookingsReport() : renderRevenueReport()}
            </div>
          )}
        </CardContent>
      </Card>
