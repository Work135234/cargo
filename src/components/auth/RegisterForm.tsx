...
const RegisterForm: React.FC<RegisterFormProps> = ({ onRegisterSuccess, onSwitchToLogin, defaultRole }) => {
  ...
  useEffect(() => {
    ...
    const checkAdmin = async () => {
      try {
        const res = await fetch('http://localhost:5000/api/admin/users?role=Admin');
  ...
  const {
    ...
    defaultValues: { role: defaultRole || 'Customer' },
  });
  ...
  const onSubmit = async (data: RegisterFormData) => {
    ...
    try {
      ...
      if (checkRes.ok && checkData.exists) {
        if (checkData.role && checkData.role !== data.role) {
          setError(`This email is already registered as ${checkData.role}. Please sign in as ${checkData.role}.`);
          setIsLoading(false);
          return;
        } else {
      ...
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            {adminExists && (
              <p className="text-xs text-gray-500">Admin registration is disabled because an admin already exists.</p>
            )}
          </div>
