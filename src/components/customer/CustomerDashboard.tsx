...
const CustomerDashboard: React.FC = () => {
  ...
  const { dispatchers } = useDispatchers(token);
  const [assigning, setAssigning] = useState(false);
  const [selectedDispatcher, setSelectedDispatcher] = useState('');
  const [assignError, setAssignError] = useState('');
  const handleAssignDispatcher = async (bookingId: string) => {
    ...
    try {
      const response = await fetch('http://localhost:5000/api/bookings/assign-dispatcher', {
        ...
        body: JSON.stringify({ bookingId, dispatcherId: selectedDispatcher }),
      });
      const result = await response.json();
      if (result.success) {
        showToast('Success', 'Dispatcher assigned successfully!');
        setSelectedDispatcher('');
        fetchCustomerBookings();
      } else {
        setAssignError(result.message || 'Failed to assign dispatcher');
      }
    } catch (err) {
      setAssignError('Failed to assign dispatcher');
    } finally {
      setAssigning(false);
    }
  };
                          ...
                          <div className="flex items-center gap-4 mt-2 text-xs text-gray-500">
                            ...
                            {booking.dispatcher && (
                              <div className="flex items-center gap-1">
                                ...
                                <span className="font-medium text-green-700">Assigned: {booking.dispatcher.name}</span>
                              </div>
                            )}
                          </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-2 text-sm">
                      ...
                      <div>
                        ...
                        <p><strong>Start Date:</strong> {selectedBooking.scheduledDate ? formatDate(selectedBooking.scheduledDate) : 'Not set'}</p>
                        <p><strong>End Date:</strong> {selectedBooking.estimatedDelivery ? formatDate(selectedBooking.estimatedDelivery) : 'Not set'}</p>
                      </div>
                    </div>
                  </div>
