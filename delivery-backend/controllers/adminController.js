...
exports.createPricingRule = async (req, res) => {
  try {
    ...
    // Only block if trying to create a new active rule for a mode that already has an active rule
    const isActive = req.body.isActive !== undefined ? req.body.isActive : true;
    if (isActive) {
      const existingActiveRule = await PricingRule.findOne({ modeOfTransport, isActive: true });
      if (existingActiveRule) {
        return res.status(400).json({
          success: false,
          message: 'Active pricing rule already exists for this transport mode'
        });
      }
    }
