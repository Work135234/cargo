// Admin: Schedule any booking
const adminScheduleBooking = async (req, res) => {
  try {
    const { bookingId, pickupDate, deliveryDate, dispatcherId } = req.body;
    if (!bookingId || !pickupDate || !deliveryDate) {
      return res.status(400).json({ success: false, message: 'Booking ID, pickup date, and delivery date are required' });
    }
    const booking = await Booking.findById(bookingId);
    if (!booking) {
      return res.status(404).json({ success: false, message: 'Booking not found' });
    }
    booking.scheduledDate = new Date(pickupDate);
    booking.estimatedDelivery = new Date(deliveryDate);
    booking.status = 'Scheduled';
    if (dispatcherId) {
      booking.dispatcher = dispatcherId;
    }
    await booking.save();
    await booking.populate('customer', 'name email');
    await notificationController.notifyStatusUpdate(booking, 'Pending', 'Scheduled');

    // Real-time: emit bookingAssigned event to dispatcher
    if (dispatcherId && global.io && global.connectedUsers) {
      const socketId = global.connectedUsers.get(dispatcherId);
      if (socketId) {
        global.io.to(socketId).emit('bookingAssigned', {
    ...
    res.json({ success: true, booking, message: 'Booking scheduled successfully' });
  } catch (error) {
    console.error('Admin schedule booking error:', error);
...
const calculateFare = async (req, res) => {
  try {
    ...
    if (distance === undefined || distance === null || !weight || !transportMode) {
      return res.status(400).json({ message: 'Missing required parameters' });
    }
...
const createBooking = async (req, res) => {
  ...
  try {
    ...
    if (!pickupAddress || !deliveryAddress || distance === undefined || distance === null || !weight || !modeOfTransport || !fare) {
...
const updateBookingStatus = async (req, res) => {
  try {
    ...
    // Hybrid notification for key events
    if (status === 'In Transit') {
      // Delivery Dispatched
      await notificationController.notifyStatusUpdate(booking, oldStatus, 'In Transit');
    } else if (status === 'Delivered') {
      // Delivery Completed
      await notificationController.notifyStatusUpdate(booking, oldStatus, 'Delivered');
    }
...
const scheduleBooking = async (req, res) => {
  try {
    const { pickupDate, deliveryDate, ...bookingData } = req.body;

    if (!pickupDate || !deliveryDate || !bookingData) {
      return res.status(400).json({ success: false, message: 'All fields are required' });
    }
