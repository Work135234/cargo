// Check if email exists and return role
...
exports.signup = async (req, res) => {
  try {
    ...
    // Generate JWT token with _id for consistency
...
exports.login = async (req, res) => {
  try {
    ...
    if (role && user.role !== role) {
      return res.status(400).json({
        success: false,
        message: `This email is registered as ${user.role}. Please sign in as ${user.role}.`
      });
    }
